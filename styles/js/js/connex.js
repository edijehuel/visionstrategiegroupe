...
$(document).ready(function ($) {
    ...
    *************************/
    ...
    if ($('#map').length && jQuery()) {
        var locations = [
              ['<strong>KAYDAN GROUPE</strong> <br>CÃ´te dâ€™Ivoire Abidjan, Cocody CitÃ© Mermoz, Rue C29<br>  Tel: 225 27 22 51 11 90<br>', 5.3370184, -3.9937101],
            ];
            ...
            success :  function(response){
                if(response=="success"){
                    jQuery("#form_contact_message").fadeIn(1000, function(){
                        jQuery("#form_contact_message").html('<div class="alert alert-success"> Votre message a Ã©tÃ© envoyÃ©, nous vous recontacterons. Merci.</div>');
                        $('#form_contact').trigger("reset");
                        resetForm(jQuery('#form_contact'));
                    });
                }else if(response=="failed"){
                    jQuery("#form_contact_message").fadeIn(1000, function(){
                        jQuery("#form_contact_message").html('<div class="alert alert-danger">Une erreur est survenue, veuillez reÃ©essayer.</div>');
                    });  
                }else{
                    jQuery("#form_contact_message").fadeIn(1000, function(){
                        jQuery("#form_contact_message").html('<div class="alert alert-danger">Le rÃ©sultat du calcul est faux. Veuillez essayer Ã  nouveau.</div>');
                    });  
                }
